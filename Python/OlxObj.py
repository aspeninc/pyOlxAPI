...
def load_olxapi(dllPath,verbose=True):
    ...
    return:
        [ OlxAPI engine version, Build Number]
    """
...
class RESULT_FLT:
    ...
    def voltage(self,obj):
        """ (ABC) Retrieve ABC PHASE post-fault voltage of a BUS,
                           or of connected BUSES of a LINE, TRANSFORMER, SWITCH, SHIFTER,SERIESRC
        Args:
    ...
    def preVoltage(self,obj):
        """ Retrieves pre-fault voltage positive sequence, line to neutral (kV)
    ...
    def preVoltagePU(self,obj):
        """ Retrieves pre-fault voltage positive sequence, line to neutral (PU)
              of a (BUS,XFMR,SHIFTER,LINE,DCLINE2,SERIESRC,SWITCH,XFMR3) """
        if type(obj) not in {BUS,XFMR,XFMR3,SHIFTER,LINE,DCLINE2,SERIESRC,SWITCH}:
    ...
    def optime(self,obj,mult,signalonly):
        ...
        return triptime.value, OlxAPI.decode(sx.value)
    @property
    def Thevenin(self):
        """ return Thevenin equivalent sequence impedance [Zp,Zn,Z0] """
        self.__pick__()
        ra = []
        ca = [OlxAPIConst.FT_dRPt,OlxAPIConst.FT_dXPt,OlxAPIConst.FT_dRNt,OlxAPIConst.FT_dXNt,OlxAPIConst.FT_dRZt,OlxAPIConst.FT_dXZt]
        for c1 in ca:
            val = c_double(0)
            if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.GetData(OlxAPIConst.HND_SC,c_int(c1),byref(val)) :
                raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
            ra.append(val.value)
    ...
    def MVA(self):
        ...
        self.__pick__()
        val = c_double(0)
        if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.GetData(OlxAPIConst.HND_SC,c_int(OlxAPIConst.FT_dMVA),byref(val)) :
            raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
        return val.value
    @property
    def XR_ratio(self):
        ...
        self.__pick__()
        res = []
        ca = [OlxAPIConst.FT_dXR,OlxAPIConst.FT_dXRANSI,OlxAPIConst.FT_dRt,OlxAPIConst.FT_dXt]
        for c1 in ca:
            val = c_double(0)
            if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.GetData(OlxAPIConst.HND_SC,c_int(c1),byref(val)) :
                if c1==OlxAPIConst.FT_dXRANSI:
                    res.append(None)
                else:
                    raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
            else:
...
class OUTAGE:
    ...
    def __check1__(self):
        ...
        if (type(self.G) not in {float,int} or self.G<0) and self.option=='SINGLE-GND':
            se+='\n  G : Admittance Outage line grounding (mho) (option=SINGLE-GND)'
            se+='\n\tRequired: >0'
...
class __DATAABSTRACT__:
    def __init__(self,hnd):
        ...
        if type(self)==TERMINAL:
            va1 = va1 - {'GUID','TAGS','JRNL','MEMO','OLNETID'}
    ...
    @property
    def OLNETID(self):
        """ (str) OLNETID for object """
        return self.getData('OLNETID')
    #
    def delete(self):
        """ (None) delete object """
        if type(self)==TERMINAL:
        ...
        if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.DeleteEquipment(self.__hnd__):
            raise Exception(OlxAPI.ErrorString())
    #
    def equals(self,o2):
    ...
    def isInList(self,la):
        """ check if object in in List/Set/tuple of object """
        __check_currFileIdx__(self)
        if type(la) in {list,set,tuple}:
            for li in la:
                if type(li) in {list,set,tuple}:
                    if self.isInList(li):
                        return True
                else:
    ...
    def changeData(self,sPara, value):
        ...
        try:
            ...
            if type(self)==RECLSR:
                ...
                paraCode = 0
        #
        if type(self)==RECLSR and sPara=='ID' and type(value)==str:
            value+='_P'
        t0 = __getTypeParaCode__(paraCode)
        #
        if paraCode>0:
            if sname=='RLYV' and sPara=='SGLONLY':
                if type(value)!=list or len(value)!=4 or value[0] not in {0,1} or value[1] not in {0,1} or value[2] not in {0,1} or value[3] not in {0,1}:
                    se ="\nin call %s.%s = "%(sname,sPara)
                    se+=("'"+str(value)+"'") if type(value)==str else str(value)
                    se+="\n\ttype(value) required : [int 0/1]*4"
                    se+="\n\tfound                : "
                    se+=("'"+str(value)+"'") if type(value)==str else str(value)
                    raise TypeError(se)
                sv=''
                ...
                value = int(sv, 2)
            #
            if not (sname=='LINE' and sPara=='MULINE'):
    ...
    def getData(self,sPara=None):
        """ Retrieves data of object
        Args:
            sPara = None or str or list(str) of parameters
                    try ='' to print in console all sPara avalaible """
        __check_currFileIdx__(self)
        #
        if sPara is None:
            r = dict()
            for s1 in self.getAttributes():
                r[s1] = self.getData(s1)
            return r
        #
        ...
        if typ1==RLYGROUP:
            if sPara in {'BACKUP','PRIMARY'} :
                ...
                return OLCase.toOBJ( __getDatai_Array__(hnd,paraCode) )
            if sPara=='TERMINAL':
        ...
        if typ1==RECLSR:
            ...
            if p2=='PH_':
                ...
                return __getOBJ__(res,sPara=sPara)
        #
        if typ1==RLYV and sPara=='SGLONLY':
            paraCode = __OLXOBJ_PARA__['RLYV']['SGLONLY'][0]
            res = __getData__(hnd,paraCode)
            va = __int2bin__(res)
            va.reverse()
            le = len(va)
            ...
            return va
        #
        if typ1==LINE and sPara=='MULINE':
            paraCode = __OLXOBJ_PARA__['LINE']['MULINE'][0]
            return OLCase.toOBJ( __getDatai_Array__(hnd,paraCode) )
        #
        try:
            paraCode = __OLXOBJ_PARA__[typ1.__name__][sPara][0]
            ...
            pass
        if sPara in self.__allAttributes__:
            if sPara=='JRNL':
            ...
            if sPara=='OLNETID':
                if typ1==BUS:
                    return "'%s' %.1fkV"%(self.getData('NAME'),self.getData('KV'))
                elif typ1 in __OLXOBJ_EQUIPMENTO__:
                    s1 =''
                    for b1 in self.BUS:
                        s1+=b1.getData('OLNETID')+'-'
                    s1 = s1[:-1]+' ' + self.getData('CID') +__OLXOBJ_EQUIPMENTOD__[typ1]
                    return s1
                elif typ1 in __OLXOBJ_BUS1O__:
                    return self.BUS.OLNETID
                elif typ1 in {LOADUNIT,GENUNIT,SHUNTUNIT}:
                    return "'%s' @ "%self.getData('CID')+ self.BUS.OLNETID
                raise Exception(' @TODO OLNETID for '%typ1.__name__)
        #
    ...
    def toString(self,option=0):
        ...
        if option==1:
            ...
            elif typ in __OLXOBJ_RELAYO2__:
        ...
        if option == 10 or 11:
            ...
            for k,v in vals.items():
                if type(v)==list:
                    ...
                    if option==10:
                        try:
                            sres+=' '+ __OLXOBJ_PARA__[type(self).__name__][k][1]
                        except:
                            pass
            #
            for k,v in vals.items():
...
class NETWORK:
    ...
    def open(self,olrFile,readonly,verbose=True):
        ...
        super().__setattr__('__currFileIdx__',__CURRENT_FILE_IDX__)
        super().__setattr__('__scope__', {'isFullNetWork':True})
        super().__setattr__('__BUS__',None)
    #
    def __getattr__(self, name):
        return self.getData(name)
    #
    ...
    def save(self,fileNew='',verbose=True):
        ...
        Args:
            ...
            verbose  : Option to print infos to the consol (True/False) """
        __check_currFileIdx__(self)
        fn = os.path.abspath(fileNew) if fileNew else self.olrFile
        if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.SaveDataFile(fn):
            raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
        if verbose:
    ...
    def findOBJ(self,objStr):
        ...
        __check_currFileIdx__(self)
        if type(objStr)==list:
            return [self.findObj(o1) for o1 in objStr]
        if type(objStr)!=str:
            return None
        hnd = c_int(0)
    ...
    #
    def getData(self,sPara=None):
        """ Retrieves data of NETWORK by sPara
        Args:
            sPara: (str) name of parameter """
        __check_currFileIdx__(self)
        if sPara is None:
            r = dict()
            for s1 in self.getAttributes():
    ...
    def applyScope(self,areaNum=None,zoneNum=None,optionTie=0,kV=[],verbose=True):
        ...
        #
        if not __checkListType__(areaNum,int):
            se+='\n\tRequired areaNum  : [int] List of area numbers'
            se+='\n\tFound (ValueError): ' +str(areaNum)
            raise ValueError(se)
        #
        if not __checkListType__(zoneNum,int):
            se+='\n\tRequired zoneNum  : [int] List of zone numbers'
            se+='\n\tFound (ValueError): ' +str(zoneNum)
            raise ValueError(se)
        #
    ...
    def findBUS(self,val1=None,val2=None):
        ...
        try:
            ...
            return None
    #
    def findLINE(self,val1=None,val2=None,val3=None,val4=None):
        """ find LINE (return None if not found):
                ...
                OLCase.findLINE(BNAME1,BNAME2,KV,CID)   OLCase.findLINE('CLAYTOR','NEVADA',132,'1') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4)
        try:
    ...
    def findDCLINE(self,val1=None,val2=None,val3=None):
        ...
        try:
            ...
            return None
    #
    def findXFMR(self,val1=None,val2=None,val3=None,val4=None,val5=None):
        """ find XFMR (return None if not found):
                ...
                OLCase.findXFMR(BNAME1,KV1,BNAME2,KV2,CID)   OLCase.findXFMR('VERMONT',132,'VERMONT',33,'1') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4,val5)
        try:
            ...
            return None
    #
    def findXFMR3(self,val1=None,val2=None,val3=None,val4=None,val5=None):
        """ find XFMR3 (return None if not found):
                ...
                OLCase.findXFMR3(BNAME1,KV1,BNAME2,KV2,CID)   OLCase.findXFMR3('NEVADA',132,'NEVADA',33,'1') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4,val5)
        try:
            ...
            return None
    #
    def findSHIFTER(self,val1=None,val2=None,val3=None,val4=None,val5=None):
        """ find SHIFTER (return None if not found):
                ...
                OLCase.findSHIFTER(BNAME1,KV1,BNAME2,KV2,CID)         OLCase.findSHIFTER('NEVADA',132,'NEVADA PST',132,'1') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4,val5)
        try:
            ...
            return None
    #
    def findSERIESRC(self,val1=None,val2=None,val3=None,val4=None):
        """ find SERIESRC (return None if not found):
                ...
                OLCase.findSERIESRC(BNAME1,BNAME2,KV,CID)   OLCase.findSERIESRC('CLAYTOR','NEVADA',132,'2') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4)
        try:
            ...
            return None
    #
    def findSWITCH(self,val1=None,val2=None,val3=None,val4=None):
        """ find SWITCH (return None if not found):
                ...
                OLCase.findSWITCH(BNAME1,BNAME2,KV,CID)   OLCase.findSWITCH('CLAYTOR','NEVADA',132,'1') """
        val1,val2,val3 = __getValFindBus__(self,val1,val2,val3,val4)
        try:
    ...
    def findTERMINAL(self,b1=None,b2=None,sType=None,CID=None):
        """ find TERMINAL (return None if not found):
            OLCase.findTERMINAL(val1,val2)
                val1: BUS
                val2 in [LINE, XFMR3, XFMR, SHIFTER, DCLINE2, SERIESRC, SWITCH]
            or
            OLCase.findTERMINAL(b1,b2,sType,CID)   OLCase.findTERMINAL(b1,b2,'LINE','1')
                ...
                CID   : (str) """
        if sType==None and CID==None:
            if type(b1)==BUS and type(b2) in __OLXOBJ_EQUIPMENTO__:
                __check_currFileIdx__(b1)
                __check_currFileIdx__(b2)
                for t1 in b1.TERMINAL:
                    if t1.EQUIPMENT.__hnd__==b2.__hnd__:
                        return t1
                return None
            se='\nOLCase.findTERMINAL(val1,val2)'
            se+="\n\tRequired val1: BUS"
            se+="\n\t         val2: "+str(__OLXOBJ_EQUIPMENTL__).replace("'",'')
            se+='\n\tFound    val1: ' +type(b1).__name__
            se+='\n\t         val2: ' +type(b2).__name__
            raise ValueError(se)
        #
    ...
    def findOBJByTag(self,tags,sObj=None):
        ...
        Args:
            - tags: Tag string
            - sObj: [] list of name of Obj type
                 examples:
                   = [] or None      => all object types
                   = ['LINE','XFMR'] => object AC LINE and 2 windings transformer
        return:
            list of object """
        if type(tags)!=str:
            if sObj is None:
                se = '\nOLCase.findOBJByTag(tags)'
            else:
        ...
        equType = c_int(0)
        equHnd = (c_int*1)(0)
        while OlxAPIConst.OLXAPI_OK==OlxAPI.FindEquipmentByTag( str(tags), equType, equHnd ):
    ...
    @property
    def ZONENO(self):
        """ [int] NETWORK list of Zone Number """
        if self.__allBus__ is None:
            super().__setattr__('__allBus__',self.getData('BUS'))
        seta = set()
        for b1 in self.__allBus__:
    ...
    def simulateFault(self,specFlt,clearPrev=0,verbose=True):
        ...
        if type(specFlt)!=SPEC_FLT or specFlt.__type__ not in {'Classical','Simultaneous','SEA'}:
            ...
            if not flag:
                ...
                raise ValueError(se)
        #
        __runSimulate__(specFlt,clearPrev,verbose)
    #
    def tapLineTool(self,t0,tapSCAP=False,verbose=False):
        ...
        Args:
            ...
            verbose  : (True/False)
                option print to the consol all details when research mainLine
        return:
            ...
            res['allPath']    = [[]] List of all TERMINAL of all path branch
        ...
        if ty0==BUS:
            ...
            for t1 in t0.TERMINAL:
                if t1.FLAG==1 and type(t1.EQUIPMENT) in {LINE,SWITCH,SERIESRC}:
                    r1 = self.tapLineTool(t1,verbose)
                    for k,v in r1.items():
                        res[k].extend(v)
            return res
        #
        ...
        if t0.FLAG!=1:
            ...
            raise Exception(se+'\n\nUnable to continue.')
        #
        ra = OlxAPILib.tapLineTool(t0.HANDLE,tapSCAP=tapSCAP,prt=verbose)
        res = dict()
...
class BUS(__DATAABSTRACT__):
    ...
    def TERMINAL(self):
        ...
        return res
    #
    def terminalTo(self,b2,sType=None,CID=None):
        ...
        if sType!=None and CID!=None:
            while OlxAPIConst.OLXAPI_OK==OlxAPI.GetBusEquipment(hnd1,c_int(OlxAPIConst.TC_BRANCH),byref(val1)):
                ...
                e1 = t1.EQUIPMENT
                if (val2==hnd2 or val3==hnd2) and type(e1).__name__==sType and e1.CID==CID:
                    return t1
            return None
        res = []
        while OlxAPIConst.OLXAPI_OK==OlxAPI.GetBusEquipment(hnd1,c_int(OlxAPIConst.TC_BRANCH),byref(val1)):
            ...
            if val2==hnd2 or val3==hnd2:
                e1 = t1.EQUIPMENT
                if (sType==None or type(e1).__name__==sType) and (CID==None or e1.CID==CID):
                    res.append(t1)
        return res
#
...
class CCGEN(__DATAABSTRACT__):
    ...
    def VLOC(self):
        """ (int) Voltage controlled current source voltage measurement location
              0-Device terminal; 1-Network side of transformer """
...
class XFMR(__DATAABSTRACT__):
    ...
    @property
    def MVA(self):
        """ [float]*3 2-winding transformer MVA1,2,3 """
    ...
    @property
    def BUS(self):
        """ [BUS] List Buses of 2-winding transformer """
        return [self.BUS1,self.BUS2]
    @property
    def RLYGROUP1(self):
        """ RLYGROUP1 of 2-winding transformer
                None if not found """
        try:
    ...
    @property
    def RLYGROUP2(self):
        """ RLYGROUP2 of 2-winding transformer
                None if not found """
        try:
    ...
    @property
    def RLYGROUP(self):
        """ [RLYGROUP] List RLYGROUPs of 2-winding transformer """
        return [self.RLYGROUP1,self.RLYGROUP2]
    @property
    def TERMINAL(self):
        """ [TERMINAL] List TERMINALs of 2-winding transformer """
        res = []
...
class XFMR3(__DATAABSTRACT__):
    ...
    @property
    def MVA(self):
        """ [float]*3 3-winding transformer MVA1,2,3 """
        return [self.getData('MVA1'),self.getData('MVA2'),self.getData('MVA3')]
    @property
    def NAME(self):
    ...
    @property
    def RLYGROUP1(self):
        """ RLYGROUP1 of 3-winding transformer
                None if not found """
        try:
            return RLYGROUP(hnd=self.getData('RLYGROUP1').__hnd__)
        except:
    ...
    @property
    def RLYGROUP2(self):
        """ RLYGROUP2 of 3-winding transformer
                None if not found """
        try:
            return RLYGROUP(hnd=self.getData('RLYGROUP2').__hnd__)
        except:
    ...
    @property
    def RLYGROUP3(self):
        """ RLYGROUP3 of 3-winding transformer
                None if not found """
        try:
    ...
    @property
    def RLYGROUP(self):
        """ [RLYGROUP] List RLYGROUPs of 3-winding transformer """
        return [self.RLYGROUP1,self.RLYGROUP2,self.RLYGROUP3]
    @property
    def TERMINAL(self):
        """ [TERMINAL] List TERMINALs of 3-winding transformer """
        res = []
        for ti in __get_OBJTERMINAL__(self):
            res.append(TERMINAL(hnd=ti.__hnd__))
        return res
...
class SHIFTER(__DATAABSTRACT__):
    ...
    @property
    def RLYGROUP(self):
        """ [RLYGROUP] List RLYGROUPs of Phase shifter """
        return [self.RLYGROUP1,self.RLYGROUP2]
    @property
    def TERMINAL(self):
        """ [TERMINAL] List TERMINALs of Phase shifter """
        res = []
        for ti in __get_OBJTERMINAL__(self):
            res.append(TERMINAL(hnd=ti.__hnd__))
        return res
...
class LINE(__DATAABSTRACT__):
    ...
    @property
    def MULINE(self):
        """ [MULINE] Line list of mutual pairs """
...
class DCLINE2(__DATAABSTRACT__):
    ...
    @property
    def TERMINAL(self):
        """ [TERMINAL] List TERMINALs of DC Line """
        res = []
        for ti in __get_OBJTERMINAL__(self):
            res.append(TERMINAL(hnd=ti.__hnd__))
        return res
...
class SERIESRC(__DATAABSTRACT__):
    ...
    @property
    def BUS(self):
        """ [BUS] List Buses of Series capacitor/reactor """
        return [self.BUS1,self.BUS2]
    @property
    def CID(self):
    ...
    @property
    def RLYGROUP1(self):
        """ RLYGROUP1 of Series capacitor/reactor
                None if not found """
        try:
            return RLYGROUP(hnd=self.getData('RLYGROUP1').__hnd__)
        except:
    ...
    @property
    def RLYGROUP2(self):
        """ RLYGROUP2 of Series capacitor/reactor
                None if not found """
        try:
            return RLYGROUP(hnd=self.getData('RLYGROUP2').__hnd__)
        except:
    ...
    @property
    def RLYGROUP(self):
        """ [RLYGROUP] List RLYGROUPs of Series capacitor/reactor """
        return [self.RLYGROUP1,self.RLYGROUP2]
    @property
    def TERMINAL(self):
        """ [TERMINAL] List TERMINALs of Series capacitor/reactor """
        res = []
        for ti in __get_OBJTERMINAL__(self):
            res.append(TERMINAL(hnd=ti.__hnd__))
        return res
...
class RLYGROUP(__DATAABSTRACT__):
    ...
    @property
    def EQUIPMENT(self):
        """ (EQUIPMENT) Relay group Equipment """
...
class RLYOCG(__DATAABSTRACT__):
    ...
    def computeRelayTime(self,current,voltage,preVoltage):
        ...
        Args:
            current    = [complexe]*5 : relay current in [A] of  phase A, B, C,
                                        and if applicable, Io currents in neutral of transformer windings P and S
            voltage    = [complexe]*3 : relay voltage in, line to neutral, in kV of phase A, B and C
            preVoltage = (complexe)   : relay pre-fault positive sequence voltage magnitude in kV, line to neutral
        return:
        ...
        return __computeRelayTime__(self,current,voltage,preVoltage)
    #
...
class RLYV(__DATAABSTRACT__):
    ...
    @property
    def OPQTY(self):
        """ (int) Voltage relay operate on voltage option:
                1-Phase-to-Neutral; 2- Phase-to-Phase; 3-3V0;4-V1;5-V2;6-VA;7-VB;8-VC;9-VBC;10-VAB;11-VCA """
    ...
    @property
    def SGLONLY(self):
        """ [int]*4 Voltage relay signal only:
                0-No check; 1-Check for [Over-voltage Instant, Over-voltage Delay, Under-voltage Instant, Under-voltage Delay] """
        return self.getData('SGLONLY')
    @property
    def UVCVR(self):
...
class TERMINAL(__DATAABSTRACT__):
    ...
    def BUS1(self):
        ...
        h = __getDatai__(self.__hnd__,OlxAPIConst.BR_nBus1Hnd)
        return BUS(hnd=h)
    @property
    def BUS2(self):
        """ (BUS) BUS2 that attached this TERMINAL """
        h = __getDatai__(self.__hnd__,OlxAPIConst.BR_nBus2Hnd)
        return BUS(hnd=h)
    @property
    def BUS3(self):
        """ (BUS) BUS3 that attached this TERMINAL """
...
__OLXOBJ_EQUIPMENTOD__ = {XFMR3:' X',XFMR:' T',SHIFTER:' P',LINE:' L',DCLINE2:' DC',SERIESRC:' S',SWITCH:' W'}
...
__OLXOBJ_RELAYO2__ = {RLYGROUP,RLYOCG,RLYOCP,FUSE,RLYDSG,RLYDSP,RLYD,RLYV,RECLSR}
...
__OLXOBJ_BUS1O__ = {LOAD,SHUNT,SVD,GEN,GENW3,GENW4,CCGEN}
...
__OLXOBJ_PARA__['SYS2'] = {\
                ...
                'AREA'      :[0                          ,'[AREA] List of AREA in NETWORK'],\
                'AREANO'    :[0                          ,'[int] List of AREA number in NETWORK'],\
                'ZONE'      :[0                          ,'[AREA] List of ZONE in NETWORK'],\
                'ZONENO'    :[0                          ,'[int] List of ZONE number in NETWORK'],\
...
__OLXOBJ_PARA__['TERMINAL'] = {\
                ...
                'BUS1'     : [0, '(BUS) BUS1 that attached this TERMINAL'],\
                'BUS2'     : [0, '(BUS) BUS2 that attached this TERMINAL'],\
                'BUS3'     : [0, '(BUS) BUS3 that attached this TERMINAL'],\
...
__OLXOBJ_PARA__['XFMR3'] = {\
                ...
                'LTCSIDE'  :[OlxAPIConst.X3_nInService    ,'(int) 3-winding transformer LTC side: 1; 2; 0'],\
                'LTCTYPE'  :[OlxAPIConst.X3_nInService    ,'(int) 3-winding transformer LTC type: 1- control voltage; 2- control MVAR'],\
                ...
                'RPM0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer RPM0'],\
                'XPM0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer XPM0'],\
                'RSM0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer RSM0'],\
                'XSM0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer XSM0'],\
                'RMG0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer RMG0'],\
                'XMG0'     :[OlxAPIConst.X3_dXst          ,'(float) 3-winding transformer XMG0'],\
                ...
                'Z0METHOD' :[OlxAPIConst.X3_nInService    ,'(int) 3-winding transformer Z0 method 1-Short circuit impedance; 2-Classical T model impedance'],\
...
__OLXOBJ_PARA__['SHIFTER'] = {\
                ...
                'ZCORRECTNO':[OlxAPIConst.PS_nInService  ,'(int) Phase shifter correct table number'],\
...
__OLXOBJ_PARA__['LINE'] = {\
                ...
                'METEREDEND':[OlxAPIConst.LN_nMeteredEnd ,'(int) Line meteted flag: 1- at Bus1; 2-at Bus2; 0-line is in a single area;'],\
                'I2T'      :[OlxAPIConst.LN_dI2T        ,'(float) I^2T rating of line in ampere^2 Sec.'],\
                ...
                'RATG'     :[OlxAPIConst.LN_vdRating    ,'[float]*4 Line ratings in A'],\
                'BUS1'     :[OlxAPIConst.LN_nBus1Hnd    ,'(BUS) Line bus 1'],\
                'BUS2'     :[OlxAPIConst.LN_nBus2Hnd    ,'(BUS) Line bus 2'],\
                'MULINE'   :[OlxAPIConst.LN_nMuPairHnd  ,'[MULINE] Line list of mutual pairs'],\
...
__OLXOBJ_PARA__['DCLINE2'] = {\
                ...
                'METEREDEND':[OlxAPIConst.DC_nMeteredEnd,'(int) DC Line meteted flag: 1- at Bus1; 2-at Bus2; 0-line is in a single area;'],\
                'MODE'   :[OlxAPIConst.DC_nInService    ,'(int) DC Line control mode'],\
...
__OLXOBJ_PARA__['RLYGROUP'] = {\
                ...
                'EQUIPMENT' :[OlxAPIConst.RG_nBranchHnd   ,'(EQUIPMENT) Relay group Equipment'],\
...
__OLXOBJ_RLYSET__['RLYOCG'] = {\
                ...
                '32QZ2R'     :[OlxAPIConst.OG_vdDirSettingV15 ,[2],3,'(float) OC ground relay direction: Z2R: Rev threshold (Sec.ohm)'],\
                '32Q50QR'    :[OlxAPIConst.OG_vdDirSettingV15 ,[2],8,'(float) OC ground relay direction: 50QR: Rev 3I2 pickup(Sec.A)'],\
                ...
                '32GZ0R'     :[OlxAPIConst.OG_vdDirSettingV15 ,[3],3,'(float) OC ground relay direction: Z0R: Rev threshold (Sec.ohm)'],\
                '32G50GR'    :[OlxAPIConst.OG_vdDirSettingV15 ,[3],7,'(float) OC ground relay direction: 50GR: Rev 3Io pickup(Sec.A)'],\
...
__OLXOBJ_RLYSET__['RLYOCP'] = {\
                ...
                '32QZ2R'     :[OlxAPIConst.OP_vdDirSettingV15 ,[2],3,'(float) OC phase relay direction: Z2R: Rev threshold (Sec.ohm)'],\
                '32Q50QR'    :[OlxAPIConst.OP_vdDirSettingV15 ,[2],8,'(float) OC phase relay direction: 50QR: Rev 3I2 pickup(Sec.A)'],\
...
__OLXOBJ_RLYSET__['RLYDSG'] = {\
                ...
                'LIBNAME'     :[OlxAPIConst.DG_sLibrary    ,'(str) DS ground relay Library'],\
...
__OLXOBJ_RLYSET__['RLYDSP'] = {\
                ...
                'LIBNAME'     :[OlxAPIConst.DP_sLibrary   ,'(str) DS phase relay Library'],\
...
__OLXOBJ_PARA__['RLYV'] = {\
                ...
                'SGLONLY'   :[OlxAPIConst.RV_nSignalOnly ,'[int 0/1]*4 Voltage relay signal only: 0-No check; 1-Check for \n\
                             [Over-voltage Instant, Over-voltage Delay, Under-voltage Instant, Under-voltage Delay]'],\
...
def __getDatai__(hnd,paraCode):
    ...
    return val1.value
#
def __getDatai_Array__(hnd,paraCode):
    res = []
    val1 = c_int(0)
...
def __getData__(hnd,paraCode):
    ...
    if type(hnd)==list :
        if type(paraCode)!=list:
            return [__getData__(hnd1,paraCode) for hnd1 in hnd]
        else:
            for hnd1 in hnd:
                r1 = [__getData__(hnd1,paraCode1) for paraCode1 in paraCode]
                res.append(r1)
    else:
...
def __changeRLYSettingOC__(rl,sPara,value):
    ...
    if len(paraCode) ==2:
        ...
        if OlxAPIConst.OLXAPI_OK==OlxAPI.SetData(c_int(rl.__hnd__),c_int(paraCode[0]),byref(val1)):
            return
        se ="\nWrite Access = NO with %s.changeSetting('%s',"%(type(rl).__name__,sPara)
        se+="'%s')"%str(value) if type(value)==str else "%s)"%str(value)
        raise Exception(se)
    else:
        polar = __getRelaySetting__(rl,'POLAR')
...
def __getRelaySettingName__(rl):
    __check_currFileIdx__(rl)
    if type(rl) not in {RLYDSG,RLYDSP}:
    ...
    return label[:nC]
#
def __getRelaySettingOC__(rl,sPara):
    if sPara is None or sPara =='':
...
def __errorParaSettingOC__(rl,polar,sPara,sc ='',value =None):
    ...
    raise AttributeError (se)
#
def __getRelaySettingDS__(rl,sPara):
    hnd1 = rl.__hnd__
    if sPara is None or sPara=='':
...
def __isInScope__(ob,scope):
    """ check if this object is in scope """
...
def __preVoltage__(obj,style): # style 1:kV; 2:PU
    vdOut1 = (c_double*3)(0.0)
    vdOut2 = (c_double*3)(0.0)
    if OlxAPIConst.OLXAPI_FAILURE==OlxAPI.GetPSCVoltage(obj.__hnd__,vdOut1,vdOut2,c_int(style)):
        raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
    #
    if type(obj) in {XFMR,SHIFTER,LINE,DCLINE2,SERIESRC,SWITCH}:
...
def __getSEA_Result__(index):
    ...
    return res
#
def __runSimulate__(specFlt,clearPrev,verbose):
...
def __computeRelayTime__(rl,current,voltage,preVoltage):
    ...
    #
    if OlxAPIConst.OLXAPI_OK != OlxAPI.ComputeRelayTime(rl.__hnd__,Imag,Iang,Vmag,Vang,c_double(VpreMag),c_double(VpreAng),pointer(opTime),pointer(opDevice)):
        raise OlxAPI.OlxAPIException(OlxAPI.ErrorString())
    #
    return opTime.value, OlxAPI.decode(opDevice.value)
#
def __getAREAZONE__(a1,az,se):
    ...
    return ra
#
def __getValFindBus__(sel,val1,val2,val3,val4,val5=None):
    if val1 and val2 and val3 and val4:
        if val5 is None:
            val1 = sel.findBUS(val1,val3)
            val2 = sel.findBUS(val2,val3)
            val3 = val4
        else:
            val1 = sel.findBUS(val1,val2)
            val2 = sel.findBUS(val3,val4)
            val3 = val5
